...
class StaffFaceAttendance extends StatefulWidget {
  const StaffFaceAttendance({super.key});

  @override
  State<StaffFaceAttendance> createState() => _StaffFaceAttendanceState();
}

class _StaffFaceAttendanceState extends State<StaffFaceAttendance> {
  ...
  Future<void> compareImages(pickedFile) async {
    try {
      List<Members> memberList = Provider.of<MembersProvider>(context, listen: false).memberList;
  ...
  void saveStaffAttendance(Members item) async {
    String url = APIData.saveStaffAttendance;
    var response = await http.post(Uri.parse(url), headers: APIData.kHeader,
      body: jsonEncode([{
        ...
        "user_type_id": item.userTypeId.toString(),
      }]),
    );
